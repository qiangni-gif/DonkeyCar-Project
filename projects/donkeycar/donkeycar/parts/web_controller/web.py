...
class LocalWebController(tornado.web.Application):
    ...
    def run_threaded(self, img_arr=None, num_records=0, current_charging_state = False):
        self.img_arr = img_arr
        self.num_records = num_records
        self.current_charging_state = current_charging_state
                ...
                for wsclient in self.wsclients:
                    try:
                        data = {
                            ...
                            'current_charging_state': self.current_charging_state
                        }
        ...
        return self.angle, self.throttle, self.mode, self.recording, self.charging_state, self.charging_state #4900 second self.charging_state is for linefollower_setting

    def run(self, img_arr=None, current_charging_state = False):
        self.img_arr = img_arr
        self.current_charging_state = current_charging_state
        return self.angle, self.throttle, self.mode, self.recording, self.charging_state
...
class DriveAPI(RequestHandler):
    ...
    def post(self):
        ...
        self.application.charging_state = data["charging_state"]
...
class WebFpv(Application):
    ...
    def update(self):
        """ Start the tornado webserver. """
        asyncio.set_event_loop(asyncio.new_event_loop())
        self.listen(self.port)#access from other pc
        IOLoop.instance().start()
