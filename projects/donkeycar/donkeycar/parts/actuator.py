...
class SunFounder_Motor_Hat:
    ...
    PWM_A = 5 #pwm channel for motor a
    PWM_B = 4 #pwm channel for motor b
    print("sunfounder motor setting")
    FORWARD = False
    BACKWARD = True
    ...
    def getPWM_throttle(self, throttle):
        ...
        if throttle == 0:
            ...
            direction = SunFounder_Motor_Hat.FORWARD
            pulse = int(dk.utils.map_range(throttle, 0, 1, self.min_pulse, self.max_pulse))
        else:
            direction = SunFounder_Motor_Hat.BACKWARD
            pulse = int(dk.utils.map_range(throttle, -1, 0, self.max_pulse, self.min_pulse))
