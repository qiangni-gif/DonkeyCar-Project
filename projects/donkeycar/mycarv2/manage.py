...
from donkeycar.parts.chargingpad.chargingpad import ChargingPad
from donkeycar.parts.LineFollower import LineFollower #4900
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single', meta=[]):
    ...
    if camera_type == "stereo":
        ...
        elif cfg.CAMERA_TYPE == "USBCAM":
            from donkeycar.parts.camera import USBCamera
            cam = USBCamera(image_w=cfg.IMAGE_W, image_h=cfg.IMAGE_H, image_d=cfg.IMAGE_DEPTH)
        else:
    ...
    if use_joystick or cfg.USE_JOYSTICK_AS_DEFAULT:
        ...
        #   threaded=True)
        V.add(ctr,
          inputs=['cam/image_array', 'tub/num_records', 'charging/current_charging_state'],
          outputs=['user/angle', 'user/throttle', 'user/mode', 'recording', 'charging/charging_state','lf/linefollower_setting'],
          threaded=True)
    
    ###################################################
    chargingpad = ChargingPad()
    V.add(chargingpad, inputs=['charging/charging_state'], outputs=['charging/current_charging_state'])
    linefollower = LineFollower()
    V.add(linefollower, inputs=['cam/image_array','user/mode','lf/linefollower_setting'],
    outputs=['pilot/angle', 'pilot/throttle','recording'])
    ###################################################
    ...
    class DriveMode:
        def run(self, mode, 
            ...
            elif mode == 'LineFollower': # 4900
               # print("DriveMode: set to LineFollower")
                return pilot_angle if pilot_angle else 0.0, pilot_throttle if pilot_throttle else 0.0
    ...
    if cfg.DONKEY_GYM:
        ...
        from donkeycar.parts.actuator import PCA9685, SunFounder_Motor_Hat, PWMSteering, PWMThrottle
        steering_controller = PCA9685(cfg.STEERING_CHANNEL)
        steering = PWMSteering(controller=steering_controller,
                           left_pulse=cfg.STEERING_LEFT_PWM,
                           right_pulse=cfg.STEERING_RIGHT_PWM)

        throttle = SunFounder_Motor_Hat(max_pulse=cfg.THROTTLE_MAX_PWM,
